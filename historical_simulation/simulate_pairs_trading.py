...
skip_start = 0
duration = 1080 # Duration in days to run the sim. If 0, no duration. Max for my file is 237 (won't error if higher though)
...
# Drawdown tracking
last_peak_value = initial_cash
last_peak_day = 0
last_trough_value = initial_cash
last_trough_day = 0
last_max_drawdown_value = 0
last_max_drawdown_peak_day = 0
last_max_drawdown_peak_value = initial_cash
last_max_drawdown_trough_day = 0
last_max_drawdown_trough_value = initial_cash
last_peak_Mastercard_price = 0
last_peak_Visa_price = 0
...
performance_chart: pd.DataFrame = pd.DataFrame(columns=["day", "date", "visa_shares", "visa_price", "mastercard_shares", "mastercard_price", "cash", "total_value"])
...
def add_daily_performance(day, date, visa_price, visa_shares, mastercard_price, mastercard_shares, cash):
...
def log_daily_performance(day: int, freq: int = 1, pad_end: bool = True):
    ...
    # Use actual calendar days elapsed (not trading-day count) so annualized
    # figures aren't inflated by the ~252-trading-days-per-365.25-day-year gap
    current_date = performance_chart[performance_chart["day"] == day]["date"].values[0]
    init_date = performance_chart[performance_chart["day"] == 0]["date"].values[0]
    calendar_days_elapsed = (pd.Timestamp(current_date) - pd.Timestamp(init_date)).days
    years_elapsed = calendar_days_elapsed / 365.25
    if years_elapsed > 0:
    ...
    # All cells are gains (e.g. a 116.1% growth ratio is shown as +16.1%), consistent with the Yearly avg column
    print(f"+{'-'*10}+{'-'*10}+{'-'*10}+{'-'*12}+")
    print(f"|{'Day ' + str(day):<10}|{'Last ' + str(freq) + 'd':<10}|{'All time':<10}|{'Yearly avg':<12}|")
    print(f"+{'-'*10}+{'-'*10}+{'-'*10}+{'-'*12}+")
    print(f"|{'Market':<10}|{format_percent(market_growth_last_freq - 1):<10}|{format_percent(market_growth_all_time - 1):<10}|{format_percent(market_yearly_avg):<12}|")
    print(f"+{'-'*10}+{'-'*10}+{'-'*10}+{'-'*12}+")
    print(f"|{'Algo':<10}|{format_percent(algo_growth_last_freq - 1):<10}|{format_percent(algo_growth_all_time - 1):<10}|{format_percent(algo_yearly_avg):<12}|")
    print(f"+{'-'*10}+{'-'*10}+{'-'*10}+{'-'*12}+")
    print(f"|{'Perf':<10}|{format_percent(algo_perf_last_freq - 1):<10}|{format_percent(algo_perf_all_time - 1):<10}|{format_percent(perf_delta_yearly):<12}|")
    print(f"+{'-'*10}+{'-'*10}+{'-'*10}+{'-'*12}+")
    if pad_end:
        print("\n\n\n")
...
for i in range(moving_average_window, len(stocks_df) - trade_delay):
    ...
    if current_day != previous_day:
        ...
        if days_passed > skip_start:
            ...
            add_daily_performance(days_processed, current_day, visa_price, visa_shares, mastercard_price, mastercard_shares, cash)
    ...
    # Update the peak and trough values
    current_value = cash + visa_shares * stocks_df[visa_price_col_name_t1][i] + mastercard_shares * stocks_df[mastercard_price_col_name_t1][i]
    if current_value > last_peak_value:
        last_peak_value = current_value
        last_peak_day = current_day
        last_peak_Mastercard_price = mastercard_price
        last_peak_Visa_price = visa_price
        last_trough_value = current_value  # Reset trough value when a new peak is found
        last_trough_day = current_day  # Reset trough day when a new peak is found
    if current_value < last_trough_value:
        last_trough_value = current_value
        last_trough_day = current_day
        drawdown = (last_peak_value - last_trough_value) / last_peak_value
        if drawdown > last_max_drawdown_value:
            last_max_drawdown_value = drawdown
            last_max_drawdown_peak_day = last_peak_day
            last_max_drawdown_peak_value = last_peak_value
            last_max_drawdown_trough_day = last_trough_day
            last_max_drawdown_trough_value = last_trough_value
            last_max_drawdown_peak_Mastercard_price = last_peak_Mastercard_price
            last_max_drawdown_peak_Visa_price = last_peak_Visa_price
            last_max_drawdown_trough_Mastercard_price = mastercard_price
            last_max_drawdown_trough_Visa_price = visa_price
            print(f"New max drawdown: {last_max_drawdown_value * 100:.2f}%, with V falling from {last_max_drawdown_peak_Visa_price} to {last_max_drawdown_trough_Visa_price} and M falling from {last_max_drawdown_peak_Mastercard_price} to {last_max_drawdown_trough_Mastercard_price} on days {last_max_drawdown_peak_day} to {last_max_drawdown_trough_day}")
    ...
    if init_run:
        ...
        simulation_end_date = current_day
        add_daily_performance(days_processed, current_day, visa_price, visa_shares, mastercard_price, mastercard_shares, cash) # Add the first day's performance
        init_run = False
...
print(f"Max drawdown: {last_max_drawdown_value * 100:.2f}%")
print(f"Drawdown peak day: {last_max_drawdown_peak_day}, trough day: {last_max_drawdown_trough_day}")
