...
if not pd.Timestamp(init_start_date).strftime("%Y-%m-%d") == init_start_date:
    raise Exception("init_start_date must be in YYYY-MM-DD format")
if not pd.Timestamp(final_end_date).strftime("%Y-%m-%d") == final_end_date:
    raise Exception("final_end_date must be in YYYY-MM-DD format")
if pd.Timestamp(init_start_date) > pd.Timestamp(final_end_date):
    raise Exception("init_start_date must be before final_end_date")
# Start date must be no more than 5 years ago
if pd.Timestamp(init_start_date) < pd.Timestamp.now() - pd.Timedelta(days=365*5):
    raise Exception("init_start_date must be no more than 5 years ago")
...
for ticker in tickers:
    file_path = Utilities.get_path_from_project_root(f"simulate/histories/downloaded/{ticker}_{init_start_date}_to_{final_end_date}.csv")
