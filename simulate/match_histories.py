...
if len(tickers) != 2:
    raise Exception("Must provide exactly 2 tickers. e.g. `python match_histories.py AMD TSM`")

# Throw error if more than one file starting with the ticker is found
matching_files = Utilities.find_files_by_pattern(Utilities.get_path_from_project_root("simulate/histories/downloaded"), f"{tickers[0]}*.csv")
if len(matching_files) != 1:
    raise Exception(f"Found {len(matching_files)} files matching {tickers[0]}*.csv. Expected 1.")
file_path_visa = matching_files[0]

matching_files = Utilities.find_files_by_pattern(Utilities.get_path_from_project_root("simulate/histories/downloaded"), f"{tickers[1]}*.csv")
if len(matching_files) != 1:
...
new_file_path = Utilities.get_path_from_project_root(f"simulate/histories/{tickers[0]}_{tickers[1]}_clean_{date_range}.csv")
